#!/usr/bin/env python3
"""Plot one adjust_bottle episode with predicted phase boundaries."""
...
from task_def.adjust_bottle import AdjustBottleProcessor
...
def infer_raw_path(hdf5_path: Path, raw_root: Optional[Path] = None) -> Optional[Path]:
    ...
    for root in roots:
        if root is None:
            continue
        candidate = root / "adjust_bottle" / setting / "data" / f"episode{episode_id}.hdf5"
        if candidate.exists():
            return candidate
    return None
...
def short_phase_label(description: str) -> str:
    text = description.lower()
    if "approach" in text or "toward" in text:
        return "Approach"
    if "grasp" in text or "close" in text or "gripper" in text:
        return "Grasp"
    if "initial" in text or "upright" in text or "back" in text:
        return "Restore"
    return "Phase"
...
def analyze_episode(
    ...
    task_processor = AdjustBottleProcessor()
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        active_side = task_processor.active_side
    ...
    velocity = vel_left if active_side == "left" else vel_right
    active_xyz = left_xyz if active_side == "left" else right_xyz
    active_z = active_xyz[:, 2] if active_xyz is not None and active_xyz.shape[1] >= 3 else None

    time_steps = np.arange(total_steps)
    descriptions = task_processor.get_subtask_descriptions_for_phases(len(checkpoints) + 1)

    fig, axes = plt.subplots(4, 1, figsize=(14, 14))
    fig.suptitle(f"adjust_bottle trajectory analysis ({hdf5_path.parent.name}, active={active_side})", fontweight="bold")
    ...
    axes[0].axhline(y=task_processor.open_gripper_threshold, color="orange", linestyle="--", alpha=0.6, label="Open threshold 0.98")
    axes[0].axhline(y=task_processor.close_gripper_threshold, color="k", linestyle="--", alpha=0.4, label="Close threshold 0.05")
    axes[0].set_ylabel("Gripper")
    axes[0].set_title("Gripper States (low=closed, high=open)")
    axes[0].legend(loc="upper right")
    axes[0].grid(True, alpha=0.3)

    if active_z is not None:
        axes[1].plot(time_steps, active_z, "m-", label=f"{active_side.title()} EEF Z", linewidth=1.5)
        axes[1].set_ylabel("Z")
        axes[1].legend(loc="upper right")
    axes[1].set_title("Active End-Effector Height")
    axes[1].grid(True, alpha=0.3)

    axes[2].plot(time_steps, velocity, "k-", label=f"{active_side.title()} Joint Velocity", alpha=0.7)
    axes[2].axhline(y=task_processor.joint_velocity_threshold, color="r", linestyle="--", alpha=0.5, label="Joint velocity threshold")
    axes[2].set_ylabel("Velocity")
    axes[2].set_title("Active Arm Joint Velocity")
    ...
    for idx in range(len(phase_edges) - 1):
        ...
        label = short_phase_label(descriptions[idx] if idx < len(descriptions) else f"Phase {idx}")
        axes[3].text(mid, 0.5, f"P{idx}: {label}", ha="center", va="center", fontsize=11, fontweight="bold")

    for cp in checkpoints:
        for ax in axes[:3]:
            ax.axvline(x=cp, color="red", linestyle="--", linewidth=1.0, alpha=0.85)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze one adjust_bottle trajectory.")

#!/usr/bin/env python3
"""Plot one beat_block_hammer episode with predicted phase boundaries."""
...
from task_def.beat_block_hammer import BeatBlockHammerProcessor
...
def short_phase_label(description: str) -> str:
    ...
    if "approach" in text or "toward" in text:
        return "Approach"
    if "grasp" in text or "hold" in text or "secure" in text:
        return "Grasp"
    if "above" in text or "move" in text or "align" in text:
        return "Move Above"
    if "strike" in text or "hit" in text or "down" in text:
        return "Strike"
    return "Phase"
...
def analyze_episode(
    ...
    fig.suptitle(f"{TASK_NAME} trajectory analysis ({hdf5_path.parent.name}, active={active_side})", fontweight="bold")
    ...
    axes[0].axhline(y=task_processor.gripper_open_value, color="orange", linestyle="--", alpha=0.6, label="Open value 1.0")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze one beat_block_hammer trajectory.")

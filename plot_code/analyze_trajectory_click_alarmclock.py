#!/usr/bin/env python3
"""Plot one click_alarmclock episode with predicted phase boundaries."""
...
from task_def.click_alarmclock import ClickAlarmclockProcessor
...
def analyze_episode(
    ...
    active_velocity = vel_left if active_side == "left" else vel_right
    z_left = left_xyz[:, 2] if left_xyz is not None else None
    z_right = right_xyz[:, 2] if right_xyz is not None else None
    active_z = z_left if active_side == "left" else z_right
    ...
    axes[2].plot(time_steps, active_velocity, "k-", label=f"{active_side.title()} Joint Velocity", alpha=0.75)
    axes[2].axhline(y=task_processor.velocity_threshold, color="r", linestyle="--", alpha=0.5, label="Velocity threshold")
    ...
    labels = ["Move Above", "Close Gripper", "Press Down"]
    colors = ["#ccffcc", "#ffffcc", "#ffcccc"]
    for idx in range(len(phase_edges) - 1):
        ...
        axes[3].text(mid, 0.5, f"P{idx}: {labels[idx]}", ha="center", va="center", fontsize=11, fontweight="bold")

    for cp in checkpoints:
        for ax in axes[:3]:
            ax.axvline(x=cp, color="red", linestyle="--", linewidth=1.0, alpha=0.85)

    if active_z is not None and checkpoints:
        axes[1].scatter(checkpoints, active_z[checkpoints], color="red", s=30, zorder=5)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze one click_alarmclock trajectory.")

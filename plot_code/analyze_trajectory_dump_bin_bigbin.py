#!/usr/bin/env python3
"""Plot one dump_bin_bigbin episode with predicted phase boundaries."""
...
from task_def.dump_bin_bigbin import DumpBinBigbinProcessor
...
def short_phase_label(description: str) -> str:
    ...
    if "place" in text:
        return "Place"
    if "pour" in text or "tilt" in text or "dump" in text:
        return "Move+Pour"
...
def analyze_episode(
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        mode = task_processor.mode
        arms = list(task_processor.active_arm_sequence)
    ...
    arms_text = ", ".join(arms) if arms else "unknown"
    fig.suptitle(f"{TASK_NAME} trajectory analysis ({hdf5_path.parent.name}, mode={mode}, arms={arms_text})", fontweight="bold")
    ...
    colors = ["#ffcccc", "#ccffcc", "#ccccff", "#ffffcc", "#ffccff", "#ccffff", "#eeeeee"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze one dump_bin_bigbin trajectory.")

#!/usr/bin/env python3
"""Plot one handover_mic episode with predicted phase boundaries."""
...
from task_def.handover_mic import HandoverMicProcessor
...
def analyze_episode(
    ...
    first_arm = task_processor.first_arm or "left"
    second_arm = task_processor.second_arm or ("right" if first_arm == "left" else "left")

    fig, axes = plt.subplots(4, 1, figsize=(16, 15))
    title = f"{TASK_NAME} trajectory analysis ({hdf5_path.parent.name}, first={first_arm})"
    ...
    colors = ["#ffcccc", "#ffd9cc", "#ffe6cc", "#ccffcc", "#d9ffcc", "#ccccff"]
    for idx in range(len(phase_edges) - 1):
        ...
        axes[3].text(mid, 0.5, f"P{idx}: {label}", ha="center", va="center", fontsize=9, fontweight="bold")

    checkpoint_names = ["c0", "c1", "c2", "c3", "c4"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze one handover_mic trajectory.")

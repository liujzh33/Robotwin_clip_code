#!/usr/bin/env python3
"""Plot one lift_pot episode with predicted phase boundaries."""
...
from task_def.lift_pot import LiftPotProcessor
...
def analyze_episode(
    ...
    task_processor = LiftPotProcessor()
    ...
    fig.suptitle(f"{TASK_NAME} trajectory analysis ({hdf5_path.parent.name})", fontweight="bold")
    ...
    for cp_idx, cp in enumerate(checkpoints):
        ...
        axes[3].text(cp, 0.92, name, ha="center", va="top", fontsize=9, color="red", fontweight="bold")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze one lift_pot trajectory.")

#!/usr/bin/env python3
"""Plot one place_bread_basket episode with predicted phase boundaries."""
...
from task_def.place_bread_basket import PlaceBreadBasketProcessor
...
def analyze_episode(
    ...
    task_processor = PlaceBreadBasketProcessor()
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        if active_arm == "dual":
            active_arm = "left" if np.sum(left_gripper < 0.95) >= np.sum(right_gripper < 0.95) else "right"
    ...
    mode = getattr(task_processor, "mode", "unknown")
    fig.suptitle(
        f"{TASK_NAME} ({mode}, {hdf5_path.parent.name}, active={active_arm})",
        fontweight="bold",
    )
    ...
    for idx in range(len(phase_edges) - 1):
        ...
        short = descriptions[idx].split(".")[0] if idx < len(descriptions) else f"Phase {idx}"
        if len(short) > 28:
            short = short[:25] + "..."
        axes[3].text(mid, 0.5, f"P{idx}", ha="center", va="center", fontsize=9, fontweight="bold")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze one place_bread_basket trajectory.")

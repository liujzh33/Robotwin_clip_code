#!/usr/bin/env python3
"""Plot one place_bread_skillet episode with predicted phase boundaries."""
...
from task_def.place_bread_skillet import PlaceBreadSkilletProcessor
...
def analyze_episode(
    ...
    task_processor = PlaceBreadSkilletProcessor()
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        bread_arm = getattr(task_processor, "bread_arm", None) or "left"
        skillet_arm = getattr(task_processor, "skillet_arm", None) or "right"
        active_arm = bread_arm
    ...
    fig.suptitle(
        f"{TASK_NAME} ({hdf5_path.parent.name}, bread={bread_arm}, skillet={skillet_arm})",
        fontweight="bold",
    )
    ...
    labels = ["Approach", "Grasp", "Adjust", "Release"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze one place_bread_skillet trajectory.")

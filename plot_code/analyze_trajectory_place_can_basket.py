#!/usr/bin/env python3
"""Plot one place_can_basket episode with predicted phase boundaries."""
...
from task_def.place_can_basket import PlaceCanBasketProcessor
...
def analyze_episode(
    ...
    task_processor = PlaceCanBasketProcessor()
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        can_arm = getattr(task_processor, "can_arm", None) or "left"
    ...
    fig.suptitle(
        f"{TASK_NAME} ({hdf5_path.parent.name}, can={can_arm}, basket={basket_arm})",
        fontweight="bold",
    )
    ...
    labels = ["ApprCan", "GraspCan", "MoveCan", "RelCan", "ApprHdl", "GraspHdl", "Lift"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze one place_can_basket trajectory.")

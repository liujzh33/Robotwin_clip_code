#!/usr/bin/env python3
"""Plot one place_dual_shoes episode with predicted phase boundaries."""
...
from task_def.place_dual_shoes import PlaceDualShoesProcessor
...
def analyze_episode(
    ...
    task_processor = PlaceDualShoesProcessor()
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        first_arm = getattr(task_processor, "first_arm", None) or "left"
        second_arm = getattr(task_processor, "second_arm", None) or "right"
    ...
    fig.suptitle(
        f"{TASK_NAME} ({hdf5_path.parent.name}, first={first_arm}, second={second_arm})",
        fontweight="bold",
    )
    ...
    labels = ["Approach", "Grasp", "Move1", "Release1", "Move2", "Release2"]
    colors = ["#ffcccc", "#ffd9cc", "#ffe6cc", "#ccffcc", "#ccf0ff", "#e6ccff"]
    ...
    if place_done is not None and 0 < place_done < total_steps:
        ...
        axes[3].text(place_done, 0.08, "place done (ref)", ha="center", va="bottom", fontsize=8, color="gray")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze one place_dual_shoes trajectory.")

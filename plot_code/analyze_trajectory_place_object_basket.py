#!/usr/bin/env python3
"""Plot one place_object_basket episode with predicted phase boundaries."""
...
from task_def.place_object_basket import PlaceObjectBasketProcessor
...
def analyze_episode(
    ...
    task_processor = PlaceObjectBasketProcessor()
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        object_arm = getattr(task_processor, "object_arm", None) or getattr(task_processor, "can_arm", None) or "left"
        basket_arm = getattr(task_processor, "basket_arm", None) or "right"
        active_gripper = left_gripper if object_arm == "left" else right_gripper
        ...
        active_xyz = left_xyz if object_arm == "left" else right_xyz
        active_z = active_xyz[:, 2] if active_xyz is not None else None
        active_joint_vel = analyzer.compute_velocity(
            qpos, arm_indices=(0, 6) if object_arm == "left" else (7, 13)
        )
    ...
    fig.suptitle(
        f"{TASK_NAME} ({hdf5_path.parent.name}, object={object_arm}, basket={basket_arm})",
        fontweight="bold",
    )
    ...
    labels = ["ApprObj", "GraspObj", "MoveObj", "RelObj", "ApprHdl", "GraspHdl", "Lift"]
    colors = ["#ffcccc", "#ffd9cc", "#ffe6cc", "#ccffcc", "#ccf0ff", "#e6f0ff", "#e6ccff"]
    ...
    checkpoint_names = [f"c{i}" for i in range(len(checkpoints))]
    ...
    if place_done is not None and 0 < place_done < total_steps:
        ...
        axes[3].text(place_done, 0.08, "release done (ref)", ha="center", va="bottom", fontsize=8, color="gray")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze one place_object_basket trajectory.")

#!/usr/bin/env python3
"""Plot one put_bottles_dustbin episode with predicted phase boundaries."""
...
from task_def.put_bottles_dustbin import PutBottlesDustbinProcessor
...
def analyze_episode(
    ...
    task_processor = PutBottlesDustbinProcessor()
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        cycle_modes = getattr(task_processor, "cycle_modes", [])
        phase_kinds = getattr(task_processor, "phase_kinds", [])
    ...
    modes_str = "+".join(cycle_modes) if cycle_modes else "?"
    fig.suptitle(
        f"{TASK_NAME} (cycles={modes_str}, {len(checkpoints)+1} phases, {hdf5_path.parent.name})",
        fontweight="bold",
    )
    ...
    descriptions = task_processor.get_subtask_descriptions_for_phases(len(phase_edges) - 1)
    colors = ["#ffcccc", "#ffd9cc", "#ffe6cc", "#ccffcc", "#ccf0ff", "#e6ccff", "#ffffcc", "#f0f0f0"]
    for idx in range(len(phase_edges) - 1):
        ...
        kind = phase_kinds[idx] if idx < len(phase_kinds) else ""
        label = kind[:12] if kind else f"P{idx}"
        axes[3].text(mid, 0.5, label, ha="center", va="center", fontsize=8, fontweight="bold")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze one put_bottles_dustbin trajectory.")

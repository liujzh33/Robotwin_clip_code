#!/usr/bin/env python3
"""Plot one put_object_cabinet episode with predicted phase boundaries."""
...
from task_def.put_object_cabinet import PutObjectCabinetProcessor
...
def analyze_episode(
    ...
    task_processor = PutObjectCabinetProcessor()
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        object_arm = getattr(task_processor, "object_arm", None) or "left"
        drawer_arm = getattr(task_processor, "drawer_arm", None) or "right"
        active_arm = object_arm
    ...
    fig.suptitle(
        f"{TASK_NAME} ({hdf5_path.parent.name}, object={object_arm}, drawer={drawer_arm})",
        fontweight="bold",
    )
    ...
    labels = ["ApprObj", "GraspObj", "ApprHdl", "GraspHdl", "PullOpen", "MoveObj", "Place"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze one put_object_cabinet trajectory.")

#!/usr/bin/env python3
"""Plot one scan_object episode with predicted phase boundaries."""
...
from task_def.scan_object import ScanObjectProcessor
...
def load_raw_endpose(raw_path: Optional[Path], total_steps: int) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
    if raw_path is None or not raw_path.exists():
        return None, None
    with h5py.File(raw_path, "r") as raw_f:
        if "endpose/left_endpose" not in raw_f or "endpose/right_endpose" not in raw_f:
            return None, None
        left_pose = raw_f["endpose/left_endpose"][()][:total_steps]
        right_pose = raw_f["endpose/right_endpose"][()][:total_steps]
    return left_pose, right_pose


def align_pose(pose: np.ndarray, total_steps: int) -> np.ndarray:
...
def analyze_episode(
    ...
    task_processor = ScanObjectProcessor()
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        left_pose, right_pose = load_raw_endpose(raw_path, total_steps)
        left_xyz = left_pose[:, :3] if left_pose is not None else None
        right_xyz = right_pose[:, :3] if right_pose is not None else None
        ...
        object_arm = task_processor.object_arm or "left"
        scanner_arm = task_processor.scanner_arm or "right"
    ...
    fig.suptitle(
        f"{TASK_NAME} ({hdf5_path.parent.name}, object={object_arm}, scanner={scanner_arm})",
        fontweight="bold",
    )
    ...
    axes[0].axhline(y=task_processor.gripper_open_value, color="orange", linestyle="--", alpha=0.5, label="Open 1.0")
    axes[0].axhline(y=task_processor.close_threshold, color="k", linestyle="--", alpha=0.4, label="Close 0.05")
    ...
    axes[2].plot(time_steps, vel_left, "b-", label="Left Joint Vel", alpha=0.75)
    axes[2].plot(time_steps, vel_right, "g-", label="Right Joint Vel", alpha=0.75)
    axes[2].axhline(y=task_processor.joint_velocity_threshold, color="r", linestyle="--", alpha=0.5)
    ...
    labels = ["Both Appr", "Both Grasp", "Lift Both", "Rotate Obj", "Scan"]
    colors = ["#ccffcc", "#ffffcc", "#ffe6cc", "#ffcccc", "#ccf0ff"]
    ...
    for cp_idx, cp in enumerate(checkpoints):
        ...
        axes[3].text(cp, 0.92, f"c{cp_idx}", ha="center", va="top", fontsize=8, color="red", fontweight="bold")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze one scan_object trajectory.")

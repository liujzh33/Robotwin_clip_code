#!/usr/bin/env python3
"""Plot one shake_bottle_horizontally episode with predicted phase boundaries."""
...
from task_def.shake_bottle_horizontally import ShakeBottleHorizontallyProcessor
...
def analyze_episode(
    ...
    task_processor = ShakeBottleHorizontallyProcessor()
    ...
    if active_xyz is not None:
        axes[2].plot(time_steps, active_xyz[:, 0], color="tab:blue", label=f"{active_arm} EEF X", linewidth=1.3)
        axes[2].plot(time_steps, active_xyz[:, 1], color="tab:orange", label=f"{active_arm} EEF Y", linewidth=1.3)
    axes[2].set_ylabel("Position")
    axes[2].set_title(f"Active Arm ({active_arm}) Horizontal EEF X/Y")
    axes[2].legend(loc="upper right")
    axes[2].grid(True, alpha=0.3)

    axes[3].plot(time_steps, vel_left, "b-", label="Left Joint Velocity", alpha=0.7)
    axes[3].plot(time_steps, vel_right, "g-", label="Right Joint Velocity", alpha=0.7)
    axes[3].axhline(y=task_processor.joint_velocity_threshold, color="r", linestyle="--", alpha=0.5, label="Velocity threshold")
    axes[3].set_ylabel("Velocity")
    axes[3].set_title("Joint Velocity")
    axes[3].legend(loc="upper right")
    axes[3].grid(True, alpha=0.3)

    axes[4].set_xlim(0, total_steps)
    axes[4].set_ylim(0, 1)
    axes[4].set_yticks([])
    axes[4].set_title(f"Predicted Phases (Total: {len(checkpoints) + 1})")
    phase_edges = [0] + checkpoints + [total_steps]
    labels = ["Approach", "Grasp", "Orient H", "Shake H"]
    colors = ["#ffcccc", "#ffd9cc", "#ffe6cc", "#ccffcc"]
    for idx in range(len(phase_edges) - 1):
        start, end = phase_edges[idx], phase_edges[idx + 1]
        mid = (start + end) / 2
        axes[4].axvspan(start, end, color=colors[idx % len(colors)], alpha=0.55)
        axes[4].axvline(x=start, color="k", linestyle="-", linewidth=0.9)
        label = labels[idx] if idx < len(labels) else f"Phase {idx}"
        axes[4].text(mid, 0.5, f"P{idx}: {label}", ha="center", va="center", fontsize=10, fontweight="bold")
    ...
    for cp_idx, cp in enumerate(checkpoints):
        ...
        axes[4].text(cp, 0.92, name, ha="center", va="top", fontsize=8, color="red", fontweight="bold")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze one shake_bottle_horizontally trajectory.")

#!/usr/bin/env python3
"""Plot one stack_bowls_three episode with predicted phase boundaries."""
...
from task_def.stack_bowls_three import StackBowlsThreeProcessor
...
def short_phase_label(description: str) -> str:
    ...
    if "first bowl" in text or ("first" in text and "bowl" in text):
        bowl = "Bowl1"
    elif "second bowl" in text or ("second" in text and "bowl" in text):
        bowl = "Bowl2"
    elif "third bowl" in text or ("third" in text and "bowl" in text):
        bowl = "Bowl3"
    else:
        bowl = "Bowl"
...
def analyze_episode(
    ...
    task_processor = StackBowlsThreeProcessor()
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        arm_sequence = list(task_processor.cycle_arms)
    ...
    arms_text = ", ".join(arm_sequence) if arm_sequence else "unknown"
    fig.suptitle(
        f"{TASK_NAME} trajectory analysis ({hdf5_path.parent.name}, arms={arms_text})",
        fontweight="bold",
    )
    ...
    checkpoint_labels = ["A1", "B1", "C1", "D1", "A2", "B2", "C2", "D2", "A3", "B3", "C3"]
    for idx in range(len(phase_edges) - 1):
        ...
        axes[3].axvline(x=start, color="k", linestyle="-", linewidth=0.8)
        label = short_phase_label(descriptions[idx] if idx < len(descriptions) else f"Phase {idx}")
        axes[3].text(mid, 0.5, f"P{idx}: {label}", ha="center", va="center", fontsize=8, fontweight="bold")

    for cp_idx, cp in enumerate(checkpoints):
        for ax in axes[:3]:
            ax.axvline(x=cp, color="red", linestyle="--", linewidth=0.8, alpha=0.8)
        if cp_idx < len(checkpoint_labels):
            axes[3].text(cp, 0.92, checkpoint_labels[cp_idx], ha="center", va="top", fontsize=7, color="red", fontweight="bold")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze one stack_bowls_three trajectory.")

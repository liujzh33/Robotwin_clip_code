#!/usr/bin/env python3
"""Plot one turn_switch episode with predicted phase boundaries."""
...
from task_def.turn_switch import TurnSwitchProcessor
...
def analyze_episode(
    ...
    task_processor = TurnSwitchProcessor()
    ...
    axes[2].plot(time_steps, vel_left, "b-", alpha=0.35, label="Left Joint Vel")
    axes[2].plot(time_steps, vel_right, "g-", alpha=0.35, label="Right Joint Vel")
    axes[2].axhline(y=task_processor.joint_velocity_threshold, color="r", linestyle="--", alpha=0.5)
    axes[2].set_ylabel("Pos / Vel")
    axes[2].set_title(f"Active Arm XY and Joint Velocity")
    ...
    labels = ["Close", "Move", "Press"]
    colors = ["#ffcccc", "#ffe6cc", "#ccffcc"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze one turn_switch trajectory.")

"""Rule-based phase segmentation for the handover_block task.

The task is modeled as a fixed left-to-right handover:
left arm approaches/grasps/moves the red block to the center, then the right arm
approaches, receives it, moves it above the blue pad, and places it.
"""
...
class HandoverBlockProcessor(BaseTaskProcessor):
    """Seven-phase splitter for a left-to-right red block handover."""
    ...
    def get_phase_checkpoints(
        ...
        **_,
        ...
        c2 = self._find_right_approach_start(
            right_z=right_z,
        ...
        right_close_start = self._find_close_start(right_gripper, start=c2, total_steps=total_steps)
        if right_close_start is None:
            right_close_start = min(total_steps - 1, c2 + max(1, (total_steps - c2) // 4))
        ...
        left_release_start = self._find_open_start(
            gripper=left_gripper,
            start=c3,
            total_steps=total_steps,
        )
        c4 = max(right_move_start, left_release_start) if left_release_start is not None else right_move_start

        c5 = self._find_place_start_by_gripper_opening(
            gripper=right_gripper,
            start=c4,
            total_steps=total_steps,
        )
    ...
    def _find_right_approach_start(
        ...
        for t in range(max(0, start), max(start, total_steps - z_window)):
            joint_ok = np.any(right_joint_velocity[t : t + self.stable_window] > self.joint_velocity_threshold)
            eef_ok = right_eef_velocity is not None and np.any(right_eef_velocity[t : t + self.stable_window] > self.eef_velocity_threshold)
            z_ok = False

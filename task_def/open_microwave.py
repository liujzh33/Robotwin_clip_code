"""Rule-based phase segmentation for the open_microwave task.

Approach the handle, grasp it, then pull horizontally to open the door. Active arm is
inferred from the first valid close event (gripper may stay closed at episode end).
"""
...
class OpenMicrowaveProcessor(BaseTaskProcessor):
    """Three-phase splitter: approach handle, grasp handle, pull door open."""
    ...
    def get_phase_checkpoints(
        ...
        **_,
        ...
        c1 = self._find_open_microwave_start_after_grasp(
            gripper=gripper,
            xyz=active_xyz,
            joint_velocity=joint_vel,
    ...
    def _find_open_microwave_start_after_grasp(
        self,
        gripper: np.ndarray,
        ...
        for t in range(close_done, max(close_done, total_steps - xy_window)):
            xy_move_ok = float(np.linalg.norm(xy[t + xy_window] - xy[t])) > self.xy_move_threshold
            ...
            if xy_move_ok or joint_ok or eef_ok:
                return int(t)

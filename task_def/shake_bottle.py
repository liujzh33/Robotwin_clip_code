"""Rule-based phase segmentation for the shake_bottle task.

Four phases: approach, grasp, lift upright, shake. Active arm is inferred from
gripper close, subsequent Z lift, and periodic shake motion.
"""
...
class ShakeBottleProcessor(BaseTaskProcessor):
    """Four-phase splitter with lift-then-shake detection."""
    ...
    def get_phase_checkpoints(
        ...
        **_,
        ...
        if active_side is not None:
            ...
            active_arm, close_start = self._find_active_arm_by_close_lift_shake(arm_data, total_steps)
        ...
        c2 = self._find_shake_start_after_lift(
            gripper=data["gripper"],
            z=data["z"],
    ...
    def _find_active_arm_by_close_lift_shake(
        self,
        arm_data: dict,
        total_steps: int,
    ) -> tuple[str, int]:
        ...
        for arm in ("left", "right"):
            ...
            shake_score = 0.0
            if self._has_periodic_shake_motion(joint_vel, close_done, total_steps):
                shake_score = 1.0
    ...
    def _find_shake_start_after_lift(
        self,
        gripper: np.ndarray,
        z: Optional[np.ndarray],
        ...
        if z is None:
            return self._fallback_shake_start(joint_vel, eef_vel, start, total_steps)
        ...
        for t in range(start, search_end):
            if gripper[t] > self.close_threshold:
                continue
            if z[t] < high_z_level:
                continue
            ...
            joint_periodic = self._has_periodic_shake_motion(joint_vel, t, total_steps)
            eef_periodic = (
                eef_vel is not None and self._has_periodic_shake_motion(eef_vel, t, total_steps, threshold=0.005)
            )
            xyz_periodic = False
            if xyz is not None and len(xyz) > t + 6:
                xyz_periodic = self._has_periodic_eef_oscillation(xyz, t, total_steps, axis=0) or (
                    self._has_periodic_eef_oscillation(xyz, t, total_steps, axis=1)
                )

            if joint_periodic or eef_periodic or xyz_periodic:
                return int(t)
    ...
    def _fallback_shake_start(
        ...
        for t in range(start, total_steps - 5):
            if self._has_periodic_shake_motion(joint_vel, t, total_steps):
                return int(t)
            if eef_vel is not None and self._has_periodic_shake_motion(eef_vel, t, total_steps, threshold=0.005):
                return int(t)
    ...
    def _has_periodic_shake_motion(
        self,
        velocity: np.ndarray,
        start: int,
        total_steps: int,
    ...
    def _has_periodic_eef_oscillation(
        self,
        xyz: np.ndarray,
        start: int,
        total_steps: int,
        ...
        segment = np.asarray(xyz[start:end, axis], dtype=np.float64)
        if len(segment) < 6:
            return False
        ...
        for idx in range(1, len(segment) - 1):
            if (segment[idx] - segment[idx - 1]) * (segment[idx + 1] - segment[idx]) < 0:
                turning_points += 1
        return turning_points >= self.min_turning_points
